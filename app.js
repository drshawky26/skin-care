...
function renderProducts(skin, brand, category) {
  ...
  const filtered = allProducts.filter(p => {
    ...
    // ===== Hair mode =====
    if (skin === "hair") {
      // لازم المنتج يكون للشعر
      if (!p.skinTypes.includes("hair")) return false;

      // category في الشيت ممكن تكون multi: "oily hair, normal hair"
      const hairCats = normalizeText(p.category)
        .split(",")
        .map(s => s.trim())
        .filter(Boolean);

      // لو اليوزر مختارش نوع شعر => اعرض كل منتجات الشعر
      if (!category) {
        ok = true;
      } else {
        // لو مختار نوع شعر => لازم يكون موجود ضمن قائمة المنتج
        ok = hairCats.includes(category);
      }

      // أمان إضافي: ممنوع allskin يظهر مع الشعر
  ...
  // فلترة نوع المنتج للبشرة (اختياري)
  if (ok && category && normalizeText(p.category) !== category) return false;
}

    if (!ok) return false;

    // فلترة الشركة (اختياري)
