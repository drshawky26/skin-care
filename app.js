...
categorySidebar.addEventListener("click", (e) => {
  ...
  // امنع document click من الإخفاء
  e.stopPropagation();

  const icon = btn.querySelector(".cat-icon");
  const labelEl = btn.querySelector(".cat-text");

  const iconSrc = icon ? icon.src : "";
  const labelText = labelEl ? labelEl.textContent.trim() : "";

  // Toggle: لو نفس الكاتيجوري معروضة، اقفل
  const same = (catPinnedImg?.src && iconSrc && catPinnedImg.src === iconSrc);
  if (!catPinnedCircle.classList.contains("hidden") && same){
    hidePinnedCategory();
    return;
  }

  showPinnedCategory(iconSrc, labelText);
});
...
const SKIN_CATEGORY_RULES = {
  ...
  oily: ["cleanser", "moisturizer", "sunscreen", "acne", "black heads", "night care","toner","wrinkles","finelines","exfoliating","viltigo","micellar water"],
  dry: ["cleanser", "moisturizer", "sunscreen", "night care","toner","wrinkles","finelines","exfoliating","viltigo","micellar water","lip balm","healing"],
  combination: ["cleanser", "moisturizer", "sunscreen", "acne", "black heads", "night care","toner","wrinkles","finelines","exfoliating","viltigo","micellar water"],
  normal: ["cleanser", "moisturizer", "sunscreen", "acne", "black heads", "night care","toner","wrinkles","finelines","exfoliating","viltigo","micellar water"],
  sensitive: ["cleanser", "moisturizer", "sunscreen", "night care","toner","wrinkles","finelines","exfoliating","viltigo","micellar water"],
};
...
function selectProductFromSearch(p){
  ...
  // لازم نعيد بناء كاتيجوري بناءً على البشرة قبل ما نحدد category
...
function applyFilters() {
  ...
  if (!skin && !brand && !category && !forcedProductId) {
...
categorySelect.onchange = () => { forcedProductId = ""; applyFilters(); };// ✅ Addon
